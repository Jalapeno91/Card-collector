...
export function renderAlbumView(coll, sub){
  ...
  wrap.innerHTML = `
    <div class="album" data-side="${view.albumMobileSide}">
      ...
      <div class="binder-pager">
        <button id="albumFirst" class="edge" title="First page" ${s===0?'disabled':''}>« First</button>
        <button id="albumPrev" ${s===0?'disabled':''}>‹ Prev</button>
        <span>${label}</span>
        <button id="albumNext" ${s>=spreads-1?'disabled':''}>Next ›</button>
        <button id="albumLast" class="edge" title="Last page" ${s>=spreads-1?'disabled':''}>Last »</button>
  ...
  const go = (target) => {
    if (target === s) return;
    const step = target > s ? 1 : -1;
    // Only a single-step move is a page turn; First and Last jump.
    if (Math.abs(target - s) === 1 && !prefersReducedMotion()) turnTo(target, step, coll, sub, leaves);
  ...
  bind('albumPrev',  () => go(s-1));
  bind('albumNext',  () => go(s+1));
